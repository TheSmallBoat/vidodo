#!/usr/bin/env bash
# WSY-05  Audio bridge E2E smoke test
# Verifies: compile → run (with scsynth backend) → trace verify
# When scsynth is not installed, verifies graceful fallback to reference backend.
set -euo pipefail
...
SHOW_ID="show-audio-bridge-smoke"
REVISION=1
...
# ── 3. Run with --backend=scsynth ───────────────────────────────────────
echo "── run start --backend=scsynth"
RUN_OUT=$("$AVCTL" run start \
  --show-id "$SHOW_ID" \
  --revision "$REVISION" \
  --backend scsynth 2>&1 || true)
...
# ── 5. Every audio event has an ack ────────────────────────────────────
if [[ -f "$TRACE_DIR/events.jsonl" ]]; then
  AUDIO_EVENTS=$(grep '"audio\.' "$TRACE_DIR/events.jsonl" | wc -l | tr -d ' ')
  AUDIO_WITH_ACK=$(grep '"audio\.' "$TRACE_DIR/events.jsonl" | grep '"ack"' | grep -v '"ack": null' | wc -l | tr -d ' ')
  check "all audio events have acks ($AUDIO_WITH_ACK/$AUDIO_EVENTS)" \
    "[[ '$AUDIO_EVENTS' -gt 0 && '$AUDIO_WITH_ACK' -eq '$AUDIO_EVENTS' ]]"
fi

# ── 6. Fallback diagnostic (if scsynth not available) ──────────────────
echo "── fallback behavior"
if echo "$RUN_OUT" | grep -q 'diag:.*fallback'; then
  check "graceful fallback diagnostic present" "true"
  echo "    (scsynth not installed — fell back to reference backend)"
else
  check "scsynth backend active (no fallback)" "true"
fi

# ── summary ──────────────────────────────────────────────────────────────
echo ""
echo "=== WSY-05 results: $PASS passed, $FAIL failed ==="
[[ "$FAIL" -eq 0 ]] || exit 1
