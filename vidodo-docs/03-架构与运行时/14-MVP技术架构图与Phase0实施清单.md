...
|                                                              |
|  [Audio Output / Instrument Plugins]                         |
|  [Visual Output Plugins]                                     |
|  [Lighting Output Plugins]                                   |
+--------------------------------------------------------------+
...
|                                                              |
|  [Audio Asset Hub] [GLSL Scene Hub] [Scene Model Hub]        |
|  [Lighting Preset Hub] [Instrument Preset Hub]               |
+--------------------------------------------------------------+
...
5. 具体硬件差异通过 Adapter Plugin Layer 承接，而不反向污染 Runtime Core。
6. 具体内容资源通过 External Resource Hubs 供给，而不要求内置在核心仓库中。
...
- Lighting Control 应为常规灯具总线与空间灯光矩阵输出保留统一 adapter plugin 接口。
- 外部资源至少按 `audio asset / glsl scene / scene model` 三类 HUB 进行组织，核心只消费其引用与元数据。
