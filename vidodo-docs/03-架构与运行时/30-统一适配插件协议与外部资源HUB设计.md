...
1. 单机内的离散拓扑。
例如一台机器上挂多屏、多声卡通道、DMX / Art-Net 网关或局域网灯控桥接。

2. 多节点分布式装置。
例如多个计算节点分别驱动不同屏幕、扬声器组或灯具组，并需要统一时序与状态同步。

对第一种情况，Vidodo 不需要额外引入消息总线。
...
Vidodo Core
  -> Adapter Plugin
  -> OS / Driver / Bridge / Device
```

也就是说，只要同一节点上的 adapter plugin 能把统一协议映射到本机设备，Core 就不必再内建一套分布式消息基础设施。

### 7.2 消息总线只在多节点装置层变成必要

当装置进入真正的多机分布式部署时，才会出现额外需求：
...
这时可以引入一个专门的分布式装置编排层，例如：

```text
Vidodo Core
  -> Adapter Contract
  -> Distributed Device Orchestrator
       -> Control Bus / State Bus / Health Bus
       -> Node Agent
       -> Concrete Device Backend
```

这里的消息总线不是产品核心抽象，而是某类部署形态下的实现技术。

### 7.3 消息总线不应成为 Core 的必选前提

如果把消息总线直接做进 Vidodo Core，会带来几个问题：

- 单机场景被迫承担不必要的复杂度
- Runtime Core 被部署拓扑反向绑定
- 不同装置项目会争夺同一套分布式语义
- MVP 会过早滑向复杂分布式系统，而不是先打通确定性编排闭环

因此，正确边界应是：

- Core 保持统一时间、统一协议、统一工件与统一证据链
- 是否需要 bus，由具体装置部署层决定
- bus 只服务装置编排，不重定义 Core 的 DSL / IR / patch 语义

### 7.4 更合理的仓库策略

对于真正复杂的离散分布式装置，更推荐采用独立仓库或独立适配项目，而不是把所有实现塞回 Vidodo 主仓库。

建议分层如下：

1. `vidodo`：核心仓库。
负责 DSL / IR / Timeline / Scheduler / Trace / Patch / Adapter Contract。

2. `vidodo-adapter-*` 或装置专项仓库。
负责某一类分布式视觉墙、扬声器矩阵、灯光矩阵的节点编排、消息总线、桥接协议和部署工具。

3. `vidodo-hub-*` 或资源仓库。
负责 GLSL、场景模型、音频资产、灯光 preset 等资源集合。

这样做的好处是：

- Core 可以长期保持精简和稳定
- 每类装置可以独立演进、独立发布、独立部署
- 不同项目可以复用同一 Adapter Contract，而不共享一套庞杂实现

### 7.5 架构结论

因此，这里的判断应明确为：

1. Vidodo Core 不应默认内建面向所有离散装置的消息总线。
2. 对单机或单节点装置，adapter plugin 直接对接 OS / 驱动 / bridge 即可。
3. 对多节点分布式装置，可以有消息总线，但它属于装置专项适配层，而不是 Core。
4. 一旦进入复杂分布式装置编排，更合理的是建设额外仓库或专项适配项目，而不是继续膨胀 Vidodo 主仓库。
