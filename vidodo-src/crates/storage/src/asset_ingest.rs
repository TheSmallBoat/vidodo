//! Asset ingestion pipeline: scan → stage → analyze → publish.
...
use crate::artifact_layout::{
    ArtifactLayout, artifact_ref, connect_registry, content_hash, dbfs_tenths,
    diagnostic_from_string, hash_string, load_asset_records, save_asset_records, short_hash, slug,
    unix_timestamp, upsert_asset, write_json,
};
...
fn load_asset_pack_manifest(source_root: &Path) -> Result<AssetPackManifest, String> {
    ...
    crate::artifact_layout::read_json(&manifest_path)
}
...
pub(crate) fn merge_tags(left: &[String], right: &[String]) -> Vec<String> {
...
mod tests {
    use super::*;
    use crate::artifact_layout::{read_json, save_asset_records, upsert_asset};
    ...
    fn ingests_assets_and_queries_registry() {
        ...
        assert_eq!(report.analysis_entries.len(), 4);

        let listed = crate::query_registry::list_assets(
            &layout,
            &crate::query_registry::AssetQuery::default(),
        )
        .unwrap();
        assert_eq!(listed.len(), 2);

        let filtered = crate::query_registry::list_assets(
            &layout,
            &crate::query_registry::AssetQuery {
        ...
        let asset =
            crate::query_registry::get_asset(&layout, &filtered[0].asset_id).unwrap().unwrap();
        assert_eq!(asset.status, "published");
        assert_eq!(asset.readiness.as_deref(), Some("compile_ready"));

        let analysis =
            crate::query_registry::list_asset_analysis(&layout, &asset.asset_id).unwrap();
        ...
        let jobs = crate::query_registry::list_asset_jobs(&layout, &asset.asset_id).unwrap();
    ...
    fn rejects_name_conflicts_within_same_ingest_batch() {
        ...
        assert!(
            crate::query_registry::list_assets(
                &layout,
                &crate::query_registry::AssetQuery::default()
            )
            .unwrap()
    ...
    fn lists_compile_assets_from_registry_policy() {
        ...
        let selection = crate::query_registry::list_compile_assets(&layout).unwrap();
